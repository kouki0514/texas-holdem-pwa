...
export function ClaudeToggle() {
  ...
  const preflopClaude = useGameStore((s) => s.preflopClaude)
  const setPreflopClaude = useGameStore((s) => s.setPreflopClaude)
        ...
        <label className="flex items-center gap-2 cursor-pointer select-none pl-2">
          <div
            className={`relative h-5 w-10 rounded-full transition-colors duration-200
              ${preflopClaude ? 'bg-purple-500' : 'bg-gray-600'}`}
            onClick={() => setPreflopClaude(!preflopClaude)}
          >
            <div
              className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform duration-200
                ${preflopClaude ? 'translate-x-5' : 'translate-x-0.5'}`}
            />
          </div>
          <span className="text-xs text-white/70">プリフロップもClaude</span>
        </label>
      )}
    </div>
  )
