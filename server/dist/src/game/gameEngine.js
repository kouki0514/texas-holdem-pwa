...
exports.isHandOver = isHandOver;
exports.isStreetOver = isStreetOver;
exports.advanceToNextStreet = advanceToNextStreet;
...
const COMMUNITY_CARDS_PER_PHASE = {
    flop: 3,
    turn: 1,
    river: 1,
};
...
// Initial State
// ──────────────────────────────────────────────────────────────────────────────
function createInitialState(players, bigBlind = 20) {
    return {
        phase: 'waiting',
        players,
        deck: [],
...
function startHand(state) {
    ...
    const rebought = clonePlayers(state.players).map((p) => p.chips === 0 ? { ...p, chips: REBUY_STACK } : p);
    ...
    for (let i = 0; i < n; i++) {
        players[i].holeCards = tempHands[i];
    }
    ...
    if (firstActor !== -1)
        players[firstActor].isTurn = true;
    return {
        ...state,
        phase: 'preflop',
        deck: dealt,
...
// Player Actions
// ──────────────────────────────────────────────────────────────────────────────
function applyAction(state, action, amount) {
...
function resolveShowdown(state) {
    ...
    for (const pot of sidePots) {
        ...
        const hands = eligible.map((p) => ({
            ...(0, handEvaluator_1.evaluateHand)(p.holeCards, state.communityCards),
            playerId: p.id,
        }));
        // Find winner(s) — support split pots (ties)
        const potWinners = hands.reduce((best, hand) => {
            const cmp = (0, handEvaluator_1.compareHandResults)(hand, best[0]);
            if (cmp > 0)
                return [hand];
            if (cmp === 0)
                return [...best, hand];
...
function buildShowdownState(state, players, winners) {
    return {
        ...state,
        phase: 'showdown',
        players,
 ...
 */
function findNextCanAct(players, fromIdx) {
 ...
 */
function findFirstPostflopActor(players, dealerIdx) {
    const n = players.length;
    for (let i = 1; i <= n; i++) {
